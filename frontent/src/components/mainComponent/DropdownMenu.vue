<template>
  <div class="dropdown dropdown-hover dropdown-bottom">
    <label
      ...
      :title="config.title"
    >
      <component :is="config.icon" class="h-4 w-4 text-base-content" />
    </label>

    <ul
      tabindex="0"
      class="dropdown-content p-2 shadow bg-base-100 rounded-box border border-base-300 w-36 max-h-64 overflow-auto"
    >
      <template v-for="(option, index) in config.options" :key="index">
        <li v-if="option.value === 'divider'" class="my-1 border-t border-base-200" />
        <li v-else>
          <a
            @click.stop.prevent="handleSelect(option.value)"
            :class="[
              'px-3 py-2 flex items-center rounded-btn transition-colors',
              config.isActive?.(option.value)
          ...
          >
            <component
              ...
              class="h-4 w-4 mr-2 text-base-content"
            />
            <span v-else :style="{ fontFamily: option.value }" class="text-sm">
              {{ option.label }}
            </span>
          </a>
        </li>
