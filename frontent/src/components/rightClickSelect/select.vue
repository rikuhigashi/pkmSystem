...
<template>
  ...
  <teleport to='body'>
    <div ref="menuRef" v-bind="$attrs" class="absolute z-50" v-if="props.menuItems.length"
      :style="{ top: `${adjustedPosition.top}px`, left: `${adjustedPosition.left}px` }" @contextmenu.prevent>
      <transition enter-active-class="duration-1000 ease-in-out transform transition-all"
